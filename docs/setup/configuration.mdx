...
Can be used if you are uploading other file formats than the [file types supported by default]. It takes a JSON input with the following format:
...
Learn more in the ["Restricted File Uploads" page].


:::note[Important note about existing MIME types]

If a MIME type is **[already defined in the source code][file types supported by default]**, you must provide the complete list of file extensions for that MIME type.
The provided list will **replace** the existing one (it will not be merged automatically.)
...
[file types supported by default]: https://github.com/saleor/saleor/blob/6af65ae16636437fece574304871705387d22f0d/saleor/settings.py#L1155-L1191
["Restricted File Uploads" page]: ../security/index.mdx#restricted-file-uploads
...
{
  "a": {
    "my-attr": ["my-value", "a value with spaces"]
  }
}
```
...
:::danger
This environment variable is deprecated and is dangerous to use, this should only be used for backward compatibility.

By setting this environment variable you may be exposing yourself to XSS attacks, if you really need to set it, consider using client-side cleaners like [DOMPurify] or [isomorphic-dompurify].

This will be removed in Saleor 3.23, please open a feature request (https://github.com/saleor/saleor/issues) so we can add support out of the box for the protocols that you need.
:::
