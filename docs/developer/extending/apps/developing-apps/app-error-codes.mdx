...
This error is returned when the requester is authenticated but lacks the permissions required to perform a given mutation (for example, `appUpdate`, `appDelete`, `appTokenCreate`, or other app management mutations).

- 🧐 **Cause**: The account or app calling the mutation does not include at least one of the permissions required by that API. Saleor blocks the operation before any additional validation runs.
- 💡 **Debugging Tips**:
  - Review the mutation's permission requirements in the API reference or the dashboard. Most app management mutations require both `AUTHENTICATED_STAFF_USER` and `MANAGE_APPS`, while some queries accept `AUTHENTICATED_APP`.
  - Ensure the requester (staff user or app) actually possesses the required permission set. For staff accounts, verify group assignments; for apps, confirm the token was generated for an app with the needed permissions.
  - If a staff user needs broader access, an administrator must grant the missing permission and retry the operation.
...
This error is raised when a staff user tries to manage an app whose permission scope is broader than their own. Saleor blocks updates, deletions, and token operations unless the requester can grant every permission that the app currently has.

- 🧐 **Cause**: The requester is not a superuser and does not hold the full set of permissions assigned to the target app (for example, the app has `MANAGE_ORDERS` but the staff user does not).
- 💡 **Debugging Tips**:
  - Inspect the app's permission list in the dashboard or via the API. Collect the formatted permission codenames (e.g., `order.manage_orders`).
  - Compare that list with the requester's permissions. Add any missing permissions to the staff user's role or ask someone who already has them to perform the action.
  - Superusers bypass this check; if appropriate, have a superuser run the mutation.
...
This error means the manifest is missing required data. Saleor validates that mandatory fields exist at multiple levels within the manifest.

- 🧐 **Cause**:
    - Mandatory top-level fields (e.g., `id`, `version`, `name`, `tokenTargetUrl`) are missing.
    - An extension definition omits required keys such as `label`, `url`, or `mount`.
    - A webhook definition omits required keys such as `name`, `targetUrl`, or `query`.
- 💡 **Debugging Tips**:
  ...
  - Verify each extension and webhook entry also contains all mandatory fields documented in Saleor's [App Manifest documentation](/developer/extending/apps/architecture/manifest.mdx).
