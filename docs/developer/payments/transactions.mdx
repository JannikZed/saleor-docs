...
To enable automatic checkout completion, update the channel checkout settings using the configuration example below. You can specify a `delay` and a `cutOffDate` to fine-tune the behavior:
<Tabs>
<TabItem value="Mutation">
```graphql
...
:::warning
If any issues occur during checkout validation, the checkout will not be completed. Validation issues include:
...
Use the `Checkout.problems` query to identify specific issues. For more details, refer to the [Checkout Problems page](/developer/checkout/problems).
:::
