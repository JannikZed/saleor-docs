...
- [Tax exemption status metadata](#tax-exemption-status-metadata) - a flag that indicates whether the app should write information about applied tax exemption to checkout and order public metadata.
...
#### Tax exemption status metadata

When enabled, the AvaTax App will write information about tax exemption applied by AvaTax to the checkout and order **public metadata**, under the `avataxExemptionStatus` key.

This metadata is intended to be consumed by storefronts or other clients to display additional UI (for example, a "tax exempt" badge).

:::note
The metadata is only written when tax exemption is applied. If tax exemption is not applied, the `avataxExemptionStatus` metadata key will be removed.
:::
...
By default, the [document code](https://developer.avalara.com/erp-integration-guide/sales-tax-badge/transactions/cert-document-codes/) is set to be equal to Saleor [`order`](api-reference/orders/objects/order.mdx) number.
...
### Ship-from address override

**Metadata key: `avataxShipFromAddress`**  
**Object: `Checkout` or `Order`**  
**Storage: private metadata**

By default, the AvaTax app uses the address configured in the AvaTax app settings as the `shipFrom` address when calculating taxes.

You can override the `shipFrom` address per checkout or order by setting **private metadata** with key `avataxShipFromAddress`. The value must be a JSON object where **all fields are strings**, including the zip code:

- `street` (string)
- `city` (string)
- `state` (string)
- `zip` (string, not a number)
- `country` (string, 2-letter ISO country code, for example `US`)

Example private metadata entry:
...
To set this value from Saleor, use the `updatePrivateMetadata` mutation and pass the checkout or order ID:

```graphql
mutation UpdateShipFromAddressPrivateMetadata {
...
- `OBJECT_ID` is the ID of the `Checkout` or `Order` object.
- The `value` is a JSON-encoded string containing the ship-from address.

:::note
This override is evaluated when AvaTax calculates taxes. Make sure the `avataxShipFromAddress` private metadata is set before the relevant tax calculation (for example, before updating delivery method or completing the checkout).
:::

:::caution
Use **private metadata** for this field. Private metadata is not exposed in the public GraphQL API and is only available to staff users and apps, which helps keep address details hidden from storefront clients.
:::

### Tax exemption status

**Metadata key: `avataxExemptionStatus`**  
**Objects: `Checkout`, `Order`**  
**Storage: public metadata**

When the *Tax exemption status metadata* option is enabled in the AvaTax App configuration, the app will update the checkout and order public metadata entry with key `avataxExemptionStatus`.

The metadata value is a JSON string with the following fields:

- `exemptAmountTotal` (number)
- `entityUseCode` (string, optional)
...
```json
{
  "key": "avataxExemptionStatus",
  "value": "{\"exemptAmountTotal\":123.45,\"entityUseCode\":\"A\",\"calculatedAt\":\"2026-01-14T12:34:56.000Z\"}"
}
```

To read it from Saleor, you can query the checkout or order `metafield`:

```graphql
query CheckoutExemptionStatus($id: ID!) {
  checkout(id: $id) {
    id
...
```graphql
query OrderExemptionStatus($id: ID!) {
  order(id: $id) {
    id
    avataxExemptionStatus: metafield(key: "avataxExemptionStatus")
  }
