...
*Stripe Link* payment method is not automatically returned by Stripe Elements. 
You need to extract this value from `PaymentElement onChange={...}` handler, while other methods are available using `elements.submit()`

See [this PR to Test Client](https://github.com/saleor/saleor-payment-apps-testclient/pull/52/files)